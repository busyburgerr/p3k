...
`doctor` возвращает `1`, если нашёл хотя бы одну блокирующую проблему, — чтобы
его можно было поставить в скрипт как ворота. Предупреждения на код выхода не
влияют. `2` означает другое: не разобрали аргументы или упали сами. В свежем
каркасе `doctor` закономерно даёт `1` — там ещё нет ни `node_modules`, ни
скопированного `.env`, — так что в дымовом прогоне падением стоит считать
только `2`. Так это и сделано в нашем CI.
